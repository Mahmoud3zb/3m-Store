...
export const requirePermission = (requiredPermission: Permission): RequestHandler => {
    return async (req, res, next) => {
        try {
            ...
            // Admin checks: user must be an admin and possess the required permission (or be primary super admin)
            if (user.role === "admin" && (user.email === "admin@gmail.com" || user.permissions.includes(requiredPermission))) {
                return next();
            }
